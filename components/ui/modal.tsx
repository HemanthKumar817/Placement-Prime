  ...
  const enhancedFooter = React.isValidElement<{ children: React.ReactNode }>(footer)
    ? React.cloneElement(footer, {
      children: React.Children.map(footer.props.children, (child, index) => {
        if (index === 0 && React.isValidElement<ButtonProps & React.RefAttributes<HTMLButtonElement>>(child)) {
...
const ModalAction = React.forwardRef<HTMLButtonElement, ButtonProps>((props, ref) => (
